...
def second_order_forward_difference(f, x, h):
    ...
    返回：
        二阶导数的近似值
    """
    return (f(x + 2*h) - 2*f(x + h) + f(x)) / h**2  # 核心差分公式
...
# 进行精度验证
for h in h_list:
    approx = second_order_forward_difference(test_func, x0, h)
    exact = exact_second_deriv(x0)
    errors.append(abs(approx - exact))  # 计算绝对误差

# 绘制收敛曲线
plt.figure(figsize=(8, 5))
plt.loglog(h_list, errors, 'go-', label='实际误差', markersize=8)  # 绿色圆点标记
plt.plot(h_list, [h for h in h_list], 'm--', label=r"O($h$)参考线", linewidth=2)
...
plt.title("二阶导数向前差分收敛性验证 (f=sin(x))", fontsize=14, pad=20)
...
# 保存和显示
plt.savefig('forward_second_order_convergence.png', dpi=300, bbox_inches='tight')
plt.show()
